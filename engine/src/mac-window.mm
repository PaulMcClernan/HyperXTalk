...
void MCMacPlatformWindow::DoUpdate(void)
{
	...
	if (MCmajorosversion >= MCOSVersionMake(10,16,0))
	{
		...
		if (MCMacPlatformIsEventCheckingEnabled() && !s_inside_focus_event)
		{
			...
			// onto the queue.
			//
			// Wait in the current runloop mode so that AppKit delivers drawRect:
			// regardless of whether we are in a normal, event-tracking, or modal context.
			//
			// Hard limit: for newly-created windows on macOS 26+ the WindowServer
			// surface registration happens at the outer runloop level and cannot be
			// delivered inside the nested runloop used by nextEventMatchingMask:.
			// drawRect: is therefore never called while we wait, producing an infinite
			// loop.  After kMaxDrawWaitIterations (≈60 ms) we fall back to
			// displayIfNeeded, which draws synchronously to the view's backing store
			// without requiring a compositor round-trip.  For all other redraw cases
			// drawRect: fires within the first iteration so the cap is never reached.
			static const int kMaxDrawWaitIterations = 3;
			NSRunLoopMode t_run_mode = [[NSRunLoop currentRunLoop] currentMode];
			if (t_run_mode == nil)
				t_run_mode = NSDefaultRunLoopMode;
			NSMutableArray *t_popped_events = [[NSMutableArray alloc] init];
			int t_draw_wait_iters = 0;
			while (m_waiting_for_draw && !MCquit && t_draw_wait_iters < kMaxDrawWaitIterations)
			{
				NSEvent *t_event;
			...
			// Fallback for windows whose initial draw cannot be delivered inside
			// the nested runloop (e.g. first draw of a new window on macOS 26).
			if (m_waiting_for_draw && !MCquit)
